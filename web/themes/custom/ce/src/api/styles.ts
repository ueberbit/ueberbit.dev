...
export const adoptStyles = (
  ...
  if (supportsAdoptingStyleSheets) {
    ...
    const oldSheet =
      import.meta.env.DEV && __hmrId
        ? sheets.find((sheet) => sheet.__hmrId === __hmrId)
        : false;
    ...
    if (oldSheet) {
      ...
      if (import.meta.env.DEV && __hmrId) styleSheet.__hmrId = __hmrId;
    ...
    const oldStyleElement =
      import.meta.env.DEV && __hmrId
        ? Array.from(existingStyleElements).find(
    ...
    if (oldStyleElement) {
      ...
      if (import.meta.env.DEV && __hmrId) styleElement.title = __hmrId
